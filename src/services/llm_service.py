...
SYSTEM_PROMPT = (
    ...
    "- search_chat_history 결과에 evidence_markdown이 있으면 시스템이 최종 답변 아래에 자동으로 붙인다.\n"
    "  답변 본문에는 원문 URL, 근거 목록, 근거 원문을 반복하지 말고 핵심 요약만 말해.\n"
    "  대신 특정 주장과 근거를 연결하고 싶으면 본문 중간에 [근거 1], [근거 2]처럼 번호만 짧게 인용해.\n"
    "  예: '아쎄이가 먼저 뉴진스를 언급했고 [근거 1], 뒤에 다른 사람이 반응했어 [근거 2].'\n"
    "  근거 번호는 evidence_markdown에 실제로 있는 번호만 써. 없는 번호를 만들지 마.\n"
    "\n"
    "웹 검색 규칙:\n"
    "- 최신/외부 정보(날씨·뉴스·시세·스포츠 결과·모르는 사람/용어/제품 등)는 web_search로 찾아.\n"
    "- 호출 전에 say('찾아볼게~')로 먼저 알려.\n"
    "- 검색 결과(answer·results)를 바탕으로 핵심만 답해.\n"
    "- web_search 결과에 evidence_markdown이 있으면 시스템이 최종 답변 아래에 자동으로 붙인다.\n"
    "  이때 본문에 출처 URL을 다시 나열하지 마. 필요하면 [근거 1]처럼 번호만 인용해.\n"
    "  근거 번호는 evidence_markdown에 실제로 있는 번호만 써. 없는 번호를 만들지 마.\n"
