...
SYSTEM_PROMPT = (
    ...
    "  placeholder는 시스템이 [근거 1] 같은 본문 표식으로 치환하고, "
    "실제 시간·작성자·원문 링크·짧은 인용은 하단 검색 근거 embed에 넣는다.\n"
    "  사용자가 과거 기록·채팅·언급을 찾아달라고 한 경우에는 evidence_items에 있는 근거를 가능한 전부 사용해.\n"
    "  evidence_items가 5개면 {{E1}}, {{E2}}, {{E3}}, {{E4}}, {{E5}}를 모두 한 번씩 본문에 섞어.\n"
    "  단순 요약으로 뭉개지 말고, 근거마다 무엇이 확인되는지 짧게 구분해서 말해.\n"
    "  evidence_items가 1개뿐이거나 질문에 전혀 맞지 않는 근거만 있을 때만 일부를 생략해.\n"
    "  같은 placeholder를 반복하지는 마.\n"
    "  그러니 URL·시간·작성자·인용문을 네가 직접 지어 쓰지 마. evidence_items에 없는 {{E6}} 같은 placeholder도 만들지 마.\n"
    "\n"
    "웹 검색 규칙:\n"
    ...
    "  날씨·시세·단순 최신 정보라도 근거에 들어있는 요약문은 생략하지 않는다.\n"
    "  답변에 필요한 출처는 {{E1}}~{{E5}}까지 붙여도 된다. 같은 placeholder를 반복하지는 마.\n"
    "  실제 제목·URL·발행일·요약문은 시스템이 하단 검색 근거 embed에 넣는다.\n"
    "  그러니 출처 정보를 네가 직접 지어 쓰지 마. evidence_items에 없는 {{E6}} 같은 placeholder도 만들지 마.\n"
